...
Avl_str *pre_AddAvl_str(Avl_str *pTree, char *chaine) {
  static int h = 0;
  return AddAvl_str(pTree, chaine, &h);
}

Avl_str *SuppMax_str(Avl_str *pTree, char **chaine);

Avl_str *DelAvl_str(Avl_str *pTree, char *chaine, int *h) {
  ...
  // parcours récursif de l’arbre
    ...
    pTree->pR = DelAvl_str(pTree->pR, chaine, h);
  } else if (strcmp(pTree->name, chaine) > 0) { // go a gauche car elem plus
                                                // ptit
    pTree->pL = DelAvl_str(pTree->pL, chaine, h);
    *h = -*h;
  }
...
Avl_str *pre_DelAvl_str(Avl_str *pTree, char *chaine) {
  static int h = 0;
  return DelAvl_str(pTree, chaine, &h);
}
